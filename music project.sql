...
use music;
/*who is the senior most emplyoee based on job title */
...
/*Who is the best customer? The customer who has spent the mos money will be declared the best customer. 
...
WITH RECURSIVE
customter_with_country AS ( SELECT customer.customer_id, first_name, last_name, billing_country, 
sum(total) AS total_spending FROM invoice
JOIN customer ON customer.customer_id= invoice.customer_id
GROUP BY 1,2,3,4
ORDER BY 2,3 DESC),
country_max_spending AS( SELECT billing_country, MAX(total_spending) AS max_spending FROM customter_with_country GROUP BY billing_country)
